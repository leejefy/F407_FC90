...
*
* DESCRIPTION:
*     Set GPIO Xï¿½s Port Stall Vector. The value in this register is sent to the transmit
*     portion of all the ports in the device when GPIO Xï¿½s Port Stall En (below) is set
*     to a one and the value on the deviceï¿½s GPIO pin matches the value in GPIO
*     Xï¿½s Port Stall Value (below) and the GPIO pin is a GPIO input. The assertion
...
*     When a portï¿½s bit is set to a one in this vector, that port or ports will stop
...
* INPUTS:
*        gpioNum - GPIO number.
*        portStallVec - GPIO Numï¿½s Port Stall Vector.
*        portStallEn - GPIO Numï¿½s Port Stall Enable.
*        portStallValue - GPIO Numï¿½s Port Stall Value.
*
* OUTPUTS:
*        None.
*
...
*
* DESCRIPTION:
*     Get GPIO Xï¿½s Port Stall Vector. The value in this register is sent to the transmit
*     portion of all the ports in the device when GPIO Xï¿½s Port Stall En (below) is set
*     to a one and the value on the deviceï¿½s GPIO pin matches the value in GPIO
*     Xï¿½s Port Stall Value (below) and the GPIO pin is a GPIO input. The assertion
*     of this vector to the ports persists as long as the function is enabled (i.e., GPIO
...
*        will take effect ï¿½nï¿½ core clocks or ï¿½nï¿½ uSec or ï¿½nï¿½ mSec (where ï¿½nï¿½ = the
...
*        o All MACï¿½s Tx and Rx paths are idle.
...
*        the portï¿½s egress queue is empty and has nothing more to send. 0x0 is a
*        special test case that will cause EEE power saving mode to be entered
*        right away when portï¿½s EEE mode is enabled.
*
* INPUTS:
