...
)
{
    ...
    {
    	printf("==MAD_FAIL=madSmiIfWriteRegister called==_phyAddr=%x,_regAddr=%x\r\n",_phyAddr,_regAddr);
        return MAD_FAIL;
    }
    *data = (MAD_U16)_data;
	printf("=[%s] %d >>==madSmiIfWriteRegister called==_phyAddr=%x,_regAddr=%x,data=%x\r\n",__FILE__,__LINE__,_phyAddr,_regAddr,*data);
    return MAD_OK;
}
...
)
{
	...
	printf("=[%s] %d >>===madSmiIfWriteRegister called==_phyAddr=%x,_regAddr=%x,_data=%x\r\n",__FILE__,__LINE__,_phyAddr,_regAddr,_data);
    return MAD_OK;
}
