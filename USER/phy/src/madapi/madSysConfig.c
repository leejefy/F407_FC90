...
)
{
	...
	MAD_DBG_INFO(("madDriverConfig dev->baseRegAddr:%x.\r\n",dev->baseRegAddr));
    /* Initialize the driver    */
    retVal = madDriverConfig(dev);
	
	MAD_DBG_INFO(("111--madDriverConfig status:%d.\r\n",retVal));
    if(retVal != MAD_OK)
    {
        MAD_DBG_ERROR(("Initialize driverConfig Failed.return %x\r\n",(retVal | MAD_API_FAIL_INIT_DRV_CFG)));
...
static MAD_BOOL fnRegister(MAD_DEV *dev, MAD_BSP_FUNCTIONS* pBSPFunctions)
{
	MAD_DBG_INFO(("fnRegister Called.\n"));
